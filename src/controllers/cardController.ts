...
import { urlChecker } from '../service/utility';
...
class CardController {
  ...
  async postCard(req: Request, res: Response) {
    try {
      ...
      } if (mongooseErr?.name === CUSTOM_ERRORS.VALIDATION_ERROR) {
      ...
      await Cards.findById(cardId)
        ...
        .then((card) => {
          if (card.owner.toString() !== owner) {
            const error = new Error(CUSTOM_ERRORS.CARD_NOT_BELONG) as ErrorWithStatusCode;
